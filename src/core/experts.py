...
class Experts(nn.Module):
    ...
    def forward(self, x, expert_indices, gates):
        batch_size, seq_len, d_model = x.shape
        if gates.shape[:2] != (batch_size, seq_len) or gates.shape[-1] != self.n_experts:
            raise ValueError(
                "gates must have shape (batch_size, seq_len, n_experts)"
            )
        x_flat = x.view(-1, d_model)
        indices_flat = expert_indices.view(-1, expert_indices.shape[-1])
        gates_flat = gates.view(-1, gates.shape[-1])
        out = torch.zeros_like(x_flat)
        for i, expert in enumerate(self.experts):
            mask = (indices_flat == i).any(dim=-1)
            if mask.any():
                weights = gates_flat[mask, i].unsqueeze(-1)
                out[mask] = out[mask] + expert(x_flat[mask]) * weights
